...
ff92Filter <- function(x) {
    # valid.subset <- x$ccm %>% group_by(permno, rebalance_date) %>% mutate(month = month(date), has_Jun = any(month == 6), has_Dec = any(month == 12)) %>% dplyr::filter(has_Jun ==
    # TRUE && has_Dec == TRUE && is_financial == 0) %>% ungroup %>% as.data.table
    ...
    # valid.subset <- x$market.dt %>% group_by(permno) %>% arrange(date) %>% mutate(count = seq(n())) %>% ungroup %>% group_by(permno, rebalance_date) %>% mutate(valid_obs =
    # any(count >= 24)) %>% dplyr::filter(valid_obs == TRUE) %>% ungroup %>% as.data.table

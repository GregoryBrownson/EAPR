# Implementations of fundamental variable computations. These functions will be internal and should NOT be exported.
...
bookEquity <- function(dt) {
    # comp Dependencies: pstkrv, pstkl, pstk, txditc, seq
...
marketEquity <- function(dt) {
    # crsp Dependencies: prc, shrout
    dt[["market_equity"]] <- abs(dt$price) * dt$shares_out
    
    dt.grouped <- group_by(dt, date, permco)
    
    max.me <- as.data.table(summarize(dt.grouped, sum_me = sum(market_equity), market_equity = max(market_equity)))
    
    dt <- merge(dt, max.me, by = c("date", "permco", "market_equity"))
    
    dt[, `:=`(market_equity = sum_me, sum_me = NULL)]
    ...
    setnames(size, c("market_equity", "log_market_equity"), c("exp_size", "size"))
...
cashFlowToPrice <- function(dt) {
    # merged dependencies: act, ib, txdc

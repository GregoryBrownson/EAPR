...
Program: M.S. in Computational Finance and Risk Management
...
%% Adjust this
I will be meeting with one or more of the mentors every week or two to discuss design or implementation related issues and provide an update on my progress. The proposed Shiny application will be developed using GitHub as the development platform. This will allow for easier management of code, collaboration with mentors, daily builds and testing of code, etc. With guidance from mentors, a set plan may be prepared for the design and implementation of each functionality to be included in the next phase, ensuring all functionalities are accounted for and tested properly during the testing portion of each phase. Furthermore, collaborator feedback will be documented (possibly using GitHub’s issues tracker) so all criticisms of the design may be accounted for. At the end of each phase, the work will be reviewed by mentors during the review phase. Any feedback, e.g. failing test cases, should be documented so that they may be fixed in the next sprint.
